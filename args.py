...
class TrainingArguments(HfTrainingArguments):
    ...
    compression_max_len: int = field(
        default=8,
        metadata={"help": "Maximum number of compression steps (continuous vectors) to unroll."}
    )
    compression_lambda: float = field(
        default=0.0,
        metadata={"help": "Weight for the expected-length regularization term during compression."}
    )
    recompress_memory: bool = field(
        default=True,
        metadata={"help": "If True, replace the current memory with the newly generated compression vectors."}
    )
    truncate_bptt_segments: int = field(
        default=1,
        metadata={"help": "Detach the memory after this many segments to limit backpropagation span."}
    )
